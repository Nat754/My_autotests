...
class TestAlertsFrameWindow:
    ...
    class TestModalPages:
        ...
        def test_get_content_large_modal(self, driver):
            modal_page = ModalPage(driver, "https://demoqa.com/modal-dialogs")
            modal_page.open()
            header_text, title_text = modal_page.get_content_from_large_modal()
            print(header_text)
            assert header_text == "Large Modal", "Modal hasn't header text"
            assert len(title_text) > 0, "Modal hasn't title text"
